    ...
    return <div>
        ...
        <div className="overflow-y-scroll max-h-[200px]">
            {strategy.coins.map((item, key) => {
                ...
                return <div className="grid grid-cols-4 gap-4 mb-2">
                    <div className="col-span-2">
                        <div className="flex gap-2 items-center">
                            <div><img className="w-[25px] h-[25px] rounded-full" src={coinFound?.icon} alt="" /></div>
                            <div>
                                <div className="text-sm">{coinFound?.name}</div>
                                <div className="text-xs">${price.toFixed(2)} {!isBuy ? <span>&nbsp; ({coinQuantity.toFixed(4)} {coinFound?.symbol?.toUpperCase()})</span> : null}</div>
                            </div>
                        </div>
                    </div>
                    <div className="text-sm font-semibold">${allocatedAmount.toFixed(2)}</div>
                    <div className="text-sm">{isBuy ? (allocatedAmount / price).toFixed(2) : sellCoinQuantity.toFixed(4)} {coinFound?.symbol?.toUpperCase()}</div>
                </div>
            })}
        </div>
        <div className="flex justify-between w-[100%] items-center">
            <div className="relative w-[200px] mt-2">
                ...
                <input type="number" value={isBuy ? amount : quantity} onChange={e => {
                    if (isBuy) setAmount(e.target.value)
                    else setQuantity(e.target.value)
                }} className="block w-full max-w-xs pr-16 pl-9 py-2 text-sm font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-300 focus:outline-none leading-relaxed" placeholder="Enter" />
                <div id="countries" className="w-10 text-gray-900 text-sm rounded-lg block absolute top-0 right-5 h-10 pl-3 border-l  focus:outline-none flex items-center">
                    {isBuy ? "USD" : strategy.code}
                </div>
            </div>
            <div>You will receive: {isBuy ? Number(amount.length ? amount : "0") / strategyRate + " " + strategy.code : (Number(quantity.length ? quantity : "0") * strategyRate).toFixed(2) + " USD"}</div>
        </div>
    </div>
}
