...
const StrategyDetails = () => {
  ...
  const portfolioRows: TableRows[][] = [
    25, 15, 15, 10, 10, 10, 5, 5, 5, 3, 2,
  ].map((val, key) => {
    return [
      {
        field: TableHeaderField.CRYPTO_INFO,
        ...
        component: (
          <div>
            <div
              className="h-2"
              style={{
                backgroundColor: colorsList[key % 20],
                width: `${(val / 25) * 100}%`,
              }}
            >
              {val}%
  ...
  return (
    <main className="flex min-h-screen flex-col items-center gap-8 px-24 py-8">
      <Header goBack={() => router.push("/leaderboard")} />
      <div className="flex gap-8 m-1">
        <div
          style={{ height: "50vh", width: "65vw", backgroundColor: "white" }}
          ...
          <div className="flex justify-between  mt-4">
            <div className="flex gap-4 items-center">
              <div className="relative">
                <input
                  type="text"
                  className="block bg-white-300 w-full max-w-xs pr-20 pl-9 py-2 text-sm font-normal shadow-xs text-gray-900 border border-gray-300 rounded-full placeholder-gray-300 focus:outline-none leading-relaxed"
                  placeholder="Enter Amount"
                />
                <div className="flex gap-2 items-center absolute right-4 bottom-2">
                  USDC
                  <Image width={20} height={20} src={usdcIcon.src} alt="" />
                </div>
              </div>
              <button className="relative group inline-flex items-center px-8 py-1.5 overflow-hidden text-lg font-medium text-customBlue border-2 border-customBlue rounded-full hover:text-white group hover:bg-gray-50">
                <span className="absolute left-0 block w-full h-0 transition-all bg-customBlue opacity-100 group-hover:h-full top-1/2 group-hover:top-0 duration-300 ease"></span>
        ...
        <div style={{ width: "25vw" }}>
          <Datatable
            ...
            rows={portfolioRows}
            customStyles={{ width: "25vw !important" }}
            hidePagination
          />
        </div>
      </div>
